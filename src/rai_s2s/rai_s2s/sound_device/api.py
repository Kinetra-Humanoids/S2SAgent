...
class SoundDeviceConfig:
    ...
    If neither `device_number` nor `device_name` is provided, the system default
    device is used.
    ...
    device_number : Optional[int], optional
        The device number for the sound device. If None, `device_name` or the system default is used. Default is None.
    device_name : Optional[str], optional
        The name of the sound device. If None or "default", the system default is used. Default is None.
...
class SoundDeviceAPI:
    def __init__(self, config: SoundDeviceConfig):
        self.device_name = config.device_name or "default"
        self.device_number: Optional[int] = None

        self.config = config
        sd.default.latency = ("low", "low")  # type: ignore
        if config.device_number is not None:
            self.device_number = config.device_number
        elif config.device_name and config.device_name != "default":
        ...
        try:
            device_data = sd.query_devices(
                ...
                kind=self._get_device_kind(),
            )
            self.device_number = int(device_data["index"])  # type: ignore
            self.device_name = str(device_data["name"])  # type: ignore
    ...
    def _get_device_kind(self) -> Optional[str]:
        if self.config.is_input and not self.config.is_output:
            return "input"
        if self.config.is_output and not self.config.is_input:
            return "output"
        return None
